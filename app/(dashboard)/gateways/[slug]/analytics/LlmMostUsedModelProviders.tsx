...
export default function LlmMostUsedModelProviders({data}:{data:any}) {
        ...
        return (
            <div className={`bg-white border border-gray-101 min-w-[150px] min-h-[80px] rounded-[.313rem] ${styles.fontMenlo}`}>
                <div className='p-2 border-b-[1px] border-gray-101'>
                    <p className='text-gray-200 font-medium text-sm'>Model</p>
                </div>
    ...
    const dataValues = (metricData: any) => {
        let metrics: any[] = [];
        metricData.forEach((d: any, index: number) => {
          metrics.push({
            id: index,
            value: d?.values?.length > 1 ? parseInt(d?.values.reduce(largeValReducer), 10) : parseInt(d?.values[0][1], 10),
            label: `${d?.metric?.model}`,
            color: index === 0 ? '#5945FF' : generateRandomColor(),
          });
        });
        ...
        metrics.forEach((item: any) => {
          if (data[item.label]) {
            data[item.label].value += item.value;
          } else {
            data[item.label] = { ...item };
          }
        });
      
        console.log("pie chart data ", Object.values(data));
    ...
    return (<>
        ...
        <div className='flex items-center justify-  w-full border border-[#F1F1F1] p-4'>
            {data?.length>0 ?(
            <PieChart                 
                sx={{
